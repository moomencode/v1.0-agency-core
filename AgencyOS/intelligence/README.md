# Operations Intelligence (Phase 4.6)
...
- **Windowed jobs**: 8 idempotent jobs compute insights per fixed UTC window
...
- **Deterministic everything**: `eventId`, `insightId`, `alertId`, `incidentId`
  and report ids are pure functions of content — the same inputs always produce
  the same outputs, across processes and restarts.
...
IntelligenceEngine (facade)      AgencyOS/intelligence/index.js
   ...
   └── buildJobSet()              8 jobs (funnel, reliability, durations, providers,
                                  budget_burn, scheduler_stats, incidents, alerts)
```
...
Design: `reports/PHASE4_6_OPERATIONS_INTELLIGENCE_DESIGN.md` (binding).
Implementation report: `reports/PHASE4_6_OPERATIONS_INTELLIGENCE_IMPLEMENTATION.md`.
